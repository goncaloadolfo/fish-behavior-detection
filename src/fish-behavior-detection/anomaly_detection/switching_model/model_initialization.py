...
from pre_processing.pre_processing_functions import background_image_estimation, video_resolution
...
def collect_motion_vectors(grid, trajectories):
    ...
    for trajectory in trajectories:
        for i in range(len(trajectory) - 1):
            ...
            next_position = np.array([trajectory[i+1][1], trajectory[i+1][2]], dtype=np.float)
...
def initialize_init_position_priors(grid, trajectories):
    ...
    for trajectory in trajectories:
        ...
        node_indexes = identify_position_node(grid, (initial_pos[1], initial_pos[2]))
        initial_position_counter[node_indexes] += 1

    for node_indexes, counter in initial_position_counter.items():
        grid.grid_matrix[node_indexes[1]][node_indexes[0]].init_position_prior = counter / len(trajectories)
    ...
    for node_indexes, node_vectors in motion_vectors.items():
        ...
        if len(data) >= grid.nr_fields:
            ...
            for i in range(grid.nr_fields):
                ...
                node.motion_vectors_priors[i] = label_count_tuples[i][1] / len(data)

        else:
            random_vectors = 2 * \
                np.random.random_sample((grid.nr_fields, 2)) - 1
            node.motion_vectors = random_vectors
            node.motion_vectors_priors = np.full((grid.nr_fields,), 1 / grid.nr_fields)
...
def test_initialization(video_path, fishes_path, nr_nodes, nr_fields):
    resolution = video_resolution(video_path)
    background_image = background_image_estimation(video_path, 300)
...
if __name__ == "__main__":
    test_initialization("resources/videos/v29.m4v",
                        "resources/detections/v29-fishes.json", 36, 4)
